...
| **Backtest** `/backtest` | The book-depth-history warning banner. A queue form. Queuing writes a `queued` row and says so; press **Run workflow** in GitHub Actions → Backtest to start it (it no longer polls — see `docs/compute_budget.md`), or a daily sweep picks it up. Selecting a queued run says "Queued — not picked up yet", not a blank panel. |
